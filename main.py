...
import app
from app import get_data_layer
...
def handle_file_download(file_path: str):
    try:
        ...
        if not file_bytes:
            return Response(
                "File not found or empty.", status_code=404, media_type="text/plain"
            )
    except Exception as e:
        error_message = str(e)
...
try:
    images_route = next(
        ...
        if getattr(route, "path", "").startswith(f"/{documents_container}/")
    )
    ...
    documents_route = next(
        route
        for route in chainlit_app.router.routes
        if getattr(route, "path", "").startswith(f"/{images_container}/")
    )
